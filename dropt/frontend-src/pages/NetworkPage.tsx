...
import { buildIpChangeSuccessChecklist } from "@/lib/opsPostchecks";
import {
  isValidGatewayIpv4,
  isValidHostIpv4,
  validateGatewayIpv4,
...
export function NetworkPage() {
  ...
  if (tab === "vlan") {
    return (
      <div className={embedded ? "px-1" : "px-8 py-8"}>
        <button
          type="button"
          className="mb-4 text-xs text-[var(--color-muted-foreground)] underline-offset-2 hover:underline"
          onClick={() => go("choice")}
        >
          ← Network Management
        </button>
  ...
  return (
    <div className={embedded ? "px-1 py-1" : "px-8 py-8"}>
      {!embedded ? <h2 className="text-xl font-semibold tracking-tight">{t("wizard_network")}</h2> : null}
      <p className={cnSub(!embedded)}>{t("network_mgmt_sub")}</p>
      <div className="mt-8 grid max-w-4xl gap-5 sm:grid-cols-3">
        <button
          type="button"
          onClick={() => go("network")}
          className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6 text-left shadow-sm transition hover:border-[var(--color-primary)] hover:bg-[var(--color-accent)]"
        >
          <p className="text-base font-semibold">{t("network_add_network")}</p>
          <p className="mt-2 text-xs leading-relaxed text-[var(--color-muted-foreground)]">
            {t("network_add_network_hint")}
          </p>
        </button>
        <button
          type="button"
          ...
          className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6 text-left shadow-sm transition hover:border-[var(--color-primary)] hover:bg-[var(--color-accent)]"
        >
          <p className="text-base font-semibold">{t("network_add_vlan")}</p>
          <p className="mt-2 text-xs leading-relaxed text-[var(--color-muted-foreground)]">
            {t("network_add_vlan_hint")}
          </p>
        </button>
        <button
          type="button"
          ...
          className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6 text-left shadow-sm transition hover:border-[var(--color-primary)] hover:bg-[var(--color-accent)]"
        >
          <p className="text-base font-semibold">{t("network_ip_change")}</p>
          <p className="mt-2 text-xs leading-relaxed text-[var(--color-muted-foreground)]">
            {t("network_ip_change_hint")}
          </p>
        </button>
      </div>
    </div>
...
function cnSub(spaced: boolean): string {
  return spaced
...
function AddNetworkForm() {
  ...
  return (
    <>
      <h2 className="text-xl font-semibold">{t("network_add_network")}</h2>
      <p className="mt-1 text-sm text-[var(--color-muted-foreground)]">{accessHint}</p>
      <form
        onSubmit={onSubmit}
        className="mt-6 max-w-xl space-y-5 rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6 shadow-sm"
      >
        ...
        <div>
          <label className="mb-1 block text-xs text-[var(--color-muted-foreground)]">Connection type</label>
        ...
        {connType === "ethernet" ? (
          ...
          <>
            <div>
              <label className="mb-1 block text-xs text-[var(--color-muted-foreground)]">Bond mode</label>
            ...
            <div>
              <label className="mb-1 block text-xs text-[var(--color-muted-foreground)]">Bond slaves</label>
              {ifacesLoading ? (
                <p className="text-xs text-[var(--color-muted-foreground)]">Yükleniyor…</p>
              ) : (
                <div className="max-h-40 space-y-1 overflow-y-auto rounded border border-[var(--color-border)] p-2">
                  {ifaces.map((i) => (
                    <label key={i.name} className="flex cursor-pointer items-center gap-2 text-sm">
                      <input
                        type="checkbox"
        ...
        <div>
          <label className="mb-1 block text-xs text-[var(--color-muted-foreground)]">
            VLAN ID <span className="opacity-70">(opsiyonel)</span>
          </label>
          <Input
            className="font-mono"
            type="number"
...
function IpChangeForm() {
  ...
  return (
    <>
      <h2 className="text-xl font-semibold">{t("network_ip_change")}</h2>
      <p className="mt-1 text-sm text-[var(--color-muted-foreground)]">{t("network_ip_change_sub")}</p>

      <div className="mt-3 w-full rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-sm text-[var(--color-foreground)]">
        {t("network_ip_change_dns_warn")}
      </div>

      <form
        onSubmit={onSubmit}
        className="mt-6 w-full space-y-5 rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6 shadow-sm"
      >
        <Input placeholder={t("talep_id")} value={talepId} onChange={(e) => setTalepId(e.target.value)} required />
...
export function VlanRedirect() {
  ...
  next.set("tab", "vlan");
  next.delete("mode");
  return <Navigate to={`/app/network?${next.toString()}`} replace />;
}

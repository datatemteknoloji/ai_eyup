...
def _latest_hosts(db: Session) -> List[HypervisorHostMetric]:
...
def _avg_host_window(db: Session, start: datetime, end: datetime) -> Dict[str, Optional[float]]:
    row = db.execute(
        text(
            """
...
def _active_alerts(db: Session, limit: int = 80) -> List[Dict[str, Any]]:
    rows = (
...
def _capacity_rows(db: Session) -> List[Dict[str, Any]]:
    sql = """
    WITH latest AS (
      SELECT DISTINCT ON (hypervisor_id, name)
             name, usage_pct, free_gb, used_gb, capacity_gb, timestamp
      FROM virt_datastore_metrics
      WHERE timestamp >= now() - interval '6 hours'
      ORDER BY hypervisor_id, name, timestamp DESC
    ),
    old AS (
      SELECT name, avg(usage_pct) AS usage_pct
      FROM virt_datastore_metrics
      WHERE timestamp >= now() - interval '31 days'
        AND timestamp <  now() - interval '29 days'
      GROUP BY name
    )
    SELECT l.name, l.usage_pct, l.free_gb, l.used_gb, l.capacity_gb,
           o.usage_pct AS usage_30d
    FROM latest l
    LEFT JOIN old o ON o.name = l.name
    ORDER BY l.usage_pct DESC NULLS LAST
    LIMIT 8
    """
    rows = _top_sql(db, sql, {})
    out = []
    for r in rows:
...
def _latest_vm_top(db: Session, column: str, extra_expr: Optional[str] = None) -> List[Dict[str, Any]]:
    if not column.replace("_", "").isalnum():
        return []
    expr = extra_expr or column
    sql = f"""
    SELECT vm_name, host_name, value, timestamp FROM (
      SELECT DISTINCT ON (vm_name)
             vm_name, host_name, {expr} AS value, timestamp
...
def build_overview(db: Session) -> Dict[str, Any]:
    ...
    vm_running = (
        ...
        .filter(
            ...
            Server.vm_power_state.ilike("%on%"),
        )
        .count()
    )
    ...
    ds_rows = _top_sql(
        ...
        """
        SELECT DISTINCT ON (hypervisor_id, name)
               name, usage_pct, read_latency_ms, write_latency_ms, timestamp
        FROM virt_datastore_metrics
    ...
    cluster_names = {h.cluster_name for h in hosts if h.cluster_name}
    connected = sum(
        1 for h in hosts
    ...
    capacity = _capacity_rows(db)

    cur = _avg_host_window(db, now - timedelta(minutes=STALE_MINUTES), now)
    ago_1h = _avg_host_window(db, now - timedelta(hours=2), now - timedelta(hours=1))
    ago_24h = _avg_host_window(db, now - timedelta(hours=25), now - timedelta(hours=23))
    ago_7d = _avg_host_window(db, now - timedelta(days=7, hours=2), now - timedelta(days=7))
    ago_30d = _avg_host_window(db, now - timedelta(days=31), now - timedelta(days=29))
    ...
    top_vm_cpu = _latest_vm_top(db, "cpu_usage_pct")
    top_vm_mem = _latest_vm_top(db, "mem_usage_pct")
    ...
    return {
        ...
        "top_consumers": {
            "vm_cpu": [
                {"name": r.get("vm_name"), "host": r.get("host_name"), "value": _r(r.get("value"))}
                for r in top_vm_cpu
            ],
            "vm_memory": [
                {"name": r.get("vm_name"), "host": r.get("host_name"), "value": _r(r.get("value"))}
            ...
            "vm_network": [
                {"name": r.get("vm_name"), "host": r.get("host_name"), "value": _r(r.get("value"), 0)}
                for r in top_vm_net
            ],
            "host_cpu": [
                {"name": h.host_name, "value": _r(h.cpu_usage_pct)} for h in hosts_by_cpu
            ],
            "host_memory": [
                {"name": h.host_name, "value": _r(h.mem_usage_pct)} for h in hosts_by_mem
            ],
        },
...
def list_objects(
    ...
    *,
    ...
    if kind == "host":
        sql = """
        SELECT DISTINCT ON (m.host_name)
               m.host_name AS name, m.host_ref AS ref, m.cluster_name,
               m.hypervisor_id, m.connection_state, m.overall_status,
        ...
        ORDER BY m.host_name, m.timestamp DESC
        LIMIT :lim
        """
        rows = _top_sql(db, sql, {
            "q": qn, "q_empty": q_empty,
            "cl": f"%{cl}%", "cl_empty": cl_empty, "lim": limit,
        })
        items = [{
            "id": r.get("name"),
            "name": r.get("name"),
            ...
            "hypervisor_id": r.get("hypervisor_id"),
            "status": r.get("connection_state") or r.get("overall_status"),
            "hint": f"CPU {_r(r.get('cpu_usage_pct'))}% · RAM {_r(r.get('mem_usage_pct'))}%",
        } for r in rows]
    ...
    if kind == "datastore":
        ...
        SELECT DISTINCT ON (name)
               name, ds_type, usage_pct, free_gb, hypervisor_id, timestamp
        FROM virt_datastore_metrics
        WHERE timestamp >= now() - interval '2 days'
          AND (:q_empty OR name ILIKE :q)
        ORDER BY name, timestamp DESC
        LIMIT :lim
        """
        rows = _top_sql(db, sql, {"q": qn, "q_empty": q_empty, "lim": limit})
        items = [{
            "id": r.get("name"),
            "name": r.get("name"),
            "kind": "datastore",
            ...
            "hypervisor_id": r.get("hypervisor_id"),
            "status": None,
            "hint": f"%{_r(r.get('usage_pct'))} · {_r(r.get('free_gb'))} GB boş",
        } for r in rows]
        return {"ok": True, "kind": kind, "count": len(items), "items": items}

    sql = """
    SELECT DISTINCT ON (vm_name)
           vm_name AS name, vm_ref AS ref, host_name, cluster_name,
           hypervisor_id, power_state, cpu_usage_pct, mem_usage_pct, timestamp
    FROM virt_vm_metrics
    WHERE vm_name IS NOT NULL
      AND timestamp >= now() - interval '2 days'
      AND (:q_empty OR vm_name ILIKE :q OR COALESCE(host_name,'') ILIKE :q
           OR COALESCE(cluster_name,'') ILIKE :q)
      AND (:cl_empty OR cluster_name ILIKE :cl OR host_name ILIKE :cl)
    ORDER BY vm_name, timestamp DESC
    LIMIT :lim
    """
    rows = _top_sql(db, sql, {
    ...
    for r in rows:
        ...
        items.append({
            ...
            "hypervisor_id": r.get("hypervisor_id"),
            "status": r.get("power_state"),
            "hint": f"{r.get('host_name') or '—'} · CPU {_r(r.get('cpu_usage_pct'))}%",
        })
    if len(items) < limit:
...
def query_series(
    ...
    *,
    ...
    def _rows_for(mins: int, buck: Optional[str]) -> List[Dict[str, Any]]:
        ...
        if buck:
            sql = f"""
            SELECT {key_col} AS name,
                   time_bucket('{buck}', timestamp) AS ts,
                   avg({column}) AS value
            FROM {table}
            WHERE timestamp >= now() - (:mins * interval '1 minute')
              AND {key_col} IN ({in_list})
            GROUP BY {key_col}, ts
            ORDER BY {key_col}, ts
            """
        else:
            sql = f"""
            SELECT {key_col} AS name, timestamp AS ts, {column} AS value
            FROM {table}
            WHERE timestamp >= now() - (:mins * interval '1 minute')
              AND {key_col} IN ({in_list})
            ORDER BY {key_col}, timestamp
            """
        return _top_sql(db, sql, qparams)

    def _to_series(rows: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    by_name = _to_series(_rows_for(minutes, bucket))
    expanded = False
    # 15/30 dk pencere sync gecikince 0–1 nokta kalıyor; çizgi için en az 8 saat.
    if any(len(by_name.get(n) or []) < 2 for n in names):
        widen = max(minutes, 480)
        if widen != minutes or bucket:
            by_name = _to_series(_rows_for(widen, None if widen <= 1440 else "1 hour"))
            expanded = True
            minutes = widen

    series = [{
        "name": n,
        "points": by_name.get(n, []),

...
def _row_dict(server: Server, inv: Optional[LinuxInventory], columns: List[str]) -> Dict[str, Any]:
    ...
    mapping = {
        ...
        "swap_usage_percent": float(inv.swap_usage_percent) if inv and inv.swap_usage_percent is not None else None,
        "cpu_iowait_percent": float(inv.cpu_iowait_percent) if inv and inv.cpu_iowait_percent is not None else None,
        "disk_io_utilization_percent": float(inv.disk_io_utilization_percent) if inv and inv.disk_io_utilization_percent is not None else None,
        "network_rx_bytes_per_sec": float(inv.network_rx_bytes_per_sec) if inv and inv.network_rx_bytes_per_sec is not None else None,
        "network_tx_bytes_per_sec": float(inv.network_tx_bytes_per_sec) if inv and inv.network_tx_bytes_per_sec is not None else None,

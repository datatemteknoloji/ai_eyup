...
def _enrich_server_from_client(client, server: Server, vm: dict, db: Session) -> None:
...
def _upsert_vm_record(
    ...
    if client and needs_enrichment:
...
def sync_hypervisor_vms(db: Session, hypervisor: Hypervisor) -> dict:
    ...
    try:
        for vm in vms:
            ...
            if _upsert_vm_record(db, hypervisor, vm, global_cred, client=client):
                synced += 1

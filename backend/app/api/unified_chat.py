...
def _build_prompt(
    ...
    # BAGLAM, sonra ONCEKI KONUSMA kısaltılır (bkz. llm_context_budget).
    ...
    _sections = budget_sections(
        ...
        log_label="UnifiedChat",
    )

    parts = [system_block, "BAGLAM:\n" + _sections["context"]]
...
async def unified_chat_stream(
    ...
    async def pipeline(payload: dict, db: Session):
        ...
        request = UnifiedChatRequest(**payload)
        usage_token = llm_usage.start()
        ...
        async def event_generator():
            try:
                ...
                async def _collect_rag():
                    ...
                    try:
                        ...
                        return await get_rag_context_for_message(
                            message, collections=_plan.rag_collections or None,
                        )
                    except Exception:
                        return {}
                ...
                prom_ctx = live_results[2] if len(live_results) > 2 and isinstance(live_results[2], str) else ""
        ...
        try:
            async for chunk in llm_usage.sse_inject_before_done(event_generator()):
                yield chunk
        finally:
            llm_usage.stop(usage_token)

...
class ErrorBoundary extends Component<Props, State> {
  ...
  public componentDidUpdate(prevProps: Props) {
    if (this.state.hasError && prevProps.resetKey !== this.props.resetKey) {
      this.setState({ hasError: false, error: undefined, errorInfo: undefined, chunkReloading: false })
    }
  }

  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('Uncaught error in React component:', error, errorInfo)
    // Deploy sonrası stale chunk: tek seferlik otomatik yenile
    if (isChunkLoadError(error)) {
      try {
        const key = 'ainew.chunkReloadAt'
        const prev = Number(sessionStorage.getItem(key) || 0)
        if (Date.now() - prev >= 15_000) {
  ...
  public render() {
    if (this.state.hasError) {
      if (this.state.chunkReloading) {
        return (
          <div className="flex h-full min-h-[40vh] items-center justify-center p-6 text-sm text-slate-400">
        ...
        <div className="flex h-full min-h-[50vh] flex-col items-center justify-center p-6 text-center">
          ...
          <p className="mb-4 max-w-md text-gray-600">
            {isChunkLoadError(this.state.error)
              ? 'Arayüz güncellenmiş olabilir. Sayfayı yenilemeniz sorunu çoğu zaman çözer.'

...
async function apiFetch(method: string, path: string, body?: unknown) {
  ...
  if (!r.ok) {
    ...
    throw new Error(e.detail ?? 'İstek başarısız')
  }
  return r.json()
}
            ...
            <div>
              ...
              {form.role === 'admin' ? (
                <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl px-4 py-3 text-xs text-slate-500 flex items-center gap-2">
                  ...
                  Admin tüm modüllere otomatik erişir.
                </div>
              ) : (
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/60">
          <h2 className="text-base font-semibold text-white flex items-center gap-2">
            <KeyRound size={16} className="text-amber-400" /> Şifre Sıfırla — @{user.username}
          </h2>
          <button onClick={onClose} className="text-slate-500 hover:text-white"><X size={18} /></button>
        </div>
        {done ? (
          ...
          <form onSubmit={submit} className="p-6 space-y-4">
            <input value={pw} onChange={e => setPw(e.target.value)} type="password" placeholder="Yeni şifre" required autoFocus
              className="w-full bg-slate-800 border border-slate-600 text-white text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-cyan-500/50" />
            <input value={pw2} onChange={e => setPw2(e.target.value)} type="password" placeholder="Tekrar girin" required
              className="w-full bg-slate-800 border border-slate-600 text-white text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-cyan-500/50" />
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-red-700/50 rounded-2xl w-full max-w-sm shadow-2xl p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-white">@{user.username} silinsin mi?</h2>
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={invalidateAll} title="Yenile"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {u.id === me?.id && (
                                <span className="text-[9px] text-cyan-400 border border-cyan-500/40 bg-cyan-500/10 px-1 py-0.5 rounded-full">Siz</span>
                              )}
                            </div>
                            ...
                            <div className="flex justify-center">
                              {isAdmin ? (
                                ...
                                <button
                                  ...
                                  disabled={isPending}
                                  title={active ? `${m.name} — kaldır` : `${m.name} — ver`}

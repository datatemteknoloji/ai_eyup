  ...
  return (
    <div className={`rounded-lg border p-3 ${accent} hover:border-opacity-60 transition-colors`}>
      ...
      <div className="mt-2 flex flex-wrap gap-2 text-[10px] text-slate-500">
        {node.position_in_rack && <span>U: {node.position_in_rack}</span>}
        {node.cpu_cores != null && variant === 'compute' && <span>{node.cpu_cores} CPU</span>}
        {node.memory_gb != null && variant === 'compute' && <span>{node.memory_gb} GB RAM</span>}
        {node.storage_tb != null && variant === 'cell' && <span>{node.storage_tb} TB</span>}
        {node.server_id && (
          <Link to={`/servers?id=${node.server_id}`} className="text-cyan-500 hover:text-cyan-400">Linux #{node.server_id}</Link>
        )}
      </div>
    </div>
  )
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500 mt-0.5">
              {[cabinet.model, cabinet.datacenter, cabinet.cabinet_label].filter(Boolean).join(' · ') || 'Exadata Rack'}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="text-right text-xs">
            <span className="text-cyan-400">{cabinet.compute_count} CN</span>
            <span className="text-slate-600 mx-1">|</span>
            <span className="text-orange-400">{cabinet.cell_count} Cell</span>
          </div>
          <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${hs.border} ${hs.text}`}>
            {cabinet.health === 'healthy' ? 'Sağlıklı' : cabinet.health === 'warning' ? 'Uyarı' : cabinet.health === 'critical' ? 'Kritik' : 'Bilinmiyor'}
          </span>
          {showSync && (
            <button
              type="button"
        ...
        <div className="px-5 pb-5">
          ...
          <div className="relative rounded-xl border-2 border-dashed border-slate-600/50 bg-slate-900/40 p-4">
            <div className="absolute top-2 left-1/2 -translate-x-1/2 text-[10px] uppercase tracking-widest text-slate-600 font-semibold">
              {cabinet.cabinet_label || cabinet.rack_name || 'KABİNET'}
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <Server size={14} className="text-cyan-400" />
                  <span className="text-xs font-semibold text-cyan-300 uppercase tracking-wide">Compute Nodes</span>
                </div>
                {cabinet.compute_nodes.length === 0 ? (
                  <div className="text-xs text-slate-600 py-6 text-center border border-dashed border-slate-700 rounded-lg">Compute node yok</div>
                ) : (
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-semibold text-orange-300 uppercase tracking-wide">Storage Cells</span>
                </div>
                {cabinet.storage_cells.length === 0 ? (
                  <div className="text-xs text-slate-600 py-6 text-center border border-dashed border-slate-700 rounded-lg">Storage cell yok</div>
                ) : (
                  <div className="space-y-2">
          ...
          {cabinet.last_sync && (
            <div className="text-[10px] text-slate-600 mt-2">Son senkron: {new Date(cabinet.last_sync).toLocaleString('tr-TR')}</div>
          )}
        </div>
      )}
    </div>
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Exadata Envanteri</h1>
            <p className="text-slate-400 text-sm">
              {allowInventoryEdit
                ? 'Compute node ve storage cell — envanter tanımlama'
                : 'Compute node ve storage cell — salt okunur görünüm'}
            </p>
            {!allowInventoryEdit && (
              <Link to="/integrations/exadata" className="text-xs text-orange-400 hover:text-orange-300 mt-1 inline-block">
                Envanter ekle → Entegrasyonlar
              </Link>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Rack / Kabinet', value: data?.total_racks ?? 0, icon: Database, color: 'text-orange-400' },
          { label: 'Compute Node', value: data?.total_compute_nodes ?? 0, icon: Server, color: 'text-cyan-400' },
          { label: 'Storage Cell', value: data?.total_storage_cells ?? 0, icon: HardDrive, color: 'text-amber-400' },
          { label: 'Sağlıklı Rack', value: cabinets.filter(c => c.health === 'healthy').length, icon: CheckCircle2, color: 'text-green-400' },
        ].map(k => (
      ...
      {isLoading ? (
        <div className="text-center py-16 text-slate-500 text-sm">Exadata envanteri yükleniyor...</div>
      ) : cabinets.length === 0 ? (
        <div className="text-center py-20 border border-dashed border-slate-700 rounded-2xl">
          ...
          <p className="text-slate-400 mb-4">Henüz Exadata rack tanımlı değil</p>
          {allowInventoryEdit ? (
            ...
            <Link to="/integrations/exadata" className="text-sm px-4 py-2 rounded-lg bg-orange-600/20 border border-orange-500/30 text-orange-300 hover:text-white inline-block">
              Entegrasyonlar → Exadata Envanter
            </Link>
          )}
        </div>
      ) : (
          ...
          {cabinets.map(c => (
            <div key={c.id}>
              ...
              {allowInventoryEdit && (
                ...
                >
                  <Plus size={11} /> {c.name} — node/cell ekle
                </button>
              )}
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-semibold">Yeni Exadata Rack</h3>
            ...
            <div className="space-y-3">
              {(['name', 'model', 'datacenter', 'cabinet_label'] as const).map(f => (
                <div key={f}>
                  <label className="text-xs text-slate-500 block mb-1">{f === 'name' ? 'Ad *' : f === 'model' ? 'Model (X8M-2)' : f === 'datacenter' ? 'Datacenter' : 'Kabinet Etiketi'}</label>
                  <input
                    value={rackForm[f]}
            ...
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setShowAddRack(false)} className="px-4 py-2 text-sm text-slate-400">İptal</button>
              <button
              ...
              >
                {createRack.isPending ? 'Kaydediliyor...' : 'Kaydet'}
              </button>
            </div>
          </div>
        </div>
      ...
      {allowInventoryEdit && addNodeRackId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-semibold">Node / Cell Ekle</h3>
            ...
            <div className="space-y-3">
              ...
              {nodeForm.role === 'compute_node' && (
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <input placeholder="RAM GB" value={nodeForm.memory_gb} onChange={e => setNodeForm(p => ({ ...p, memory_gb: e.target.value }))} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" />
                </div>
              )}
              {nodeForm.role === 'storage_cell' && (
                <input placeholder="Storage TB" value={nodeForm.storage_tb} onChange={e => setNodeForm(p => ({ ...p, storage_tb: e.target.value }))} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" />
              )}
            </div>
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setAddNodeRackId(null)} className="px-4 py-2 text-sm text-slate-400">İptal</button>
              <button onClick={() => addNode.mutate()} disabled={!nodeForm.name || addNode.isPending} className="px-4 py-2 text-sm bg-orange-600 hover:bg-orange-500 text-white rounded-lg disabled:opacity-50">
                {addNode.isPending ? 'Ekleniyor...' : 'Ekle'}
              </button>
            </div>
          </div>
        </div>

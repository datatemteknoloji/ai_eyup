const API_BASE = "/api/dropt";

function apiUrl(path: string): string {
  // Dropt client paths are "/api/..." — strip leading /api so nginx /api/dropt/ → :8001/api/
...
export function terminalWsUrl(serverId: number, params: Record<string, string>): string {
  ...
  return `${p}://${window.location.host}/api/dropt/terminal/ws/${serverId}?${qs}`;
}

export function jobEventsWsUrl(jobId: number, token: string): string {
  const qs = new URLSearchParams({ token });
  const p = window.location.protocol === "https:" ? "wss" : "ws";
  return `${p}://${window.location.host}/api/dropt/jobs/ws/${jobId}?${qs}`;
}
